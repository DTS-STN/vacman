...
--changeset system:cleanup-expired-cd-city-nov-2025 dbms:mssql,h2
DELETE FROM PROFILE_CITY WHERE CITY_ID IN (SELECT ID FROM CD_CITY WHERE EXPIRY_DATE IS NOT NULL);
DELETE FROM REQUEST_CITY WHERE CITY_ID IN (SELECT ID FROM CD_CITY WHERE EXPIRY_DATE IS NOT NULL);
