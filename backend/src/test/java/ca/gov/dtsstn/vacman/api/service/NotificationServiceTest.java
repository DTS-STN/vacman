...
class NotificationServiceTest {
	...
	@Test
	@DisplayName("Test send Job Opportunity notification English")
	void sendJobOpportunityNotificationEnglishSuccess() {
		when(restTemplate.postForObject(eq("/email"), any(Map.class), eq(NotificationReceipt.class)))
			.thenReturn(ImmutableNotificationReceipt.builder().build());

		final var result = notificationService.sendJobOpportunityNotification(
			"test@example.com",
			123L,
			"Test Request",
			lookupCodes.languages().english());
	...
	@Test
	@DisplayName("Test send Job Opportunity notification French")
	void sendJobOpportunityNotificationFrenchSuccess() {
		when(restTemplate.postForObject(eq("/email"), any(Map.class), eq(NotificationReceipt.class)))
			.thenReturn(ImmutableNotificationReceipt.builder().build());
	...
	@Test
	@DisplayName("Test send Job Opportunity notification to multiple emails with English language")
	void sendJobOpportunityNotificationMultipleEnglishSuccess() {
		when(restTemplate.postForObject(eq("/email"), any(Map.class), eq(NotificationReceipt.class)))
			.thenReturn(ImmutableNotificationReceipt.builder().build());
	...
	@Test
	@DisplayName("Test send Job Opportunity notification to multiple emails with French language")
	void sendJobOpportunityNotificationMultipleFrenchSuccess() {
		when(restTemplate.postForObject(eq("/email"), any(Map.class), eq(NotificationReceipt.class)))
			.thenReturn(ImmutableNotificationReceipt.builder().build());
