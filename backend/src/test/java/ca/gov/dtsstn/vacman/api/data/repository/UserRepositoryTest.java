...
class UserRepositoryTest {
	...
	void whenFindByNetworkName_givenUserExists_thenReturnUser() {
		...
		assertThat(savedUser).isNotNull();
		assertThat(savedUser.getId()).isNotNull();
		assertThat(savedUser.getActiveDirectoryId()).isEqualTo(testNetworkName);

		final var foundUserOptional = userRepository.findByActiveDirectoryId(testNetworkName);

		assertThat(foundUserOptional).isPresent();

		final var foundUser = foundUserOptional.get();
		assertThat(foundUser.getId()).isEqualTo(savedUser.getId());
		assertThat(foundUser.getActiveDirectoryId()).isEqualTo(testNetworkName);
