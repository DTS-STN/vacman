...
- support (Node 24) — CLI for Microsoft Graph
...
- Frontend (Node ≥24, pnpm ≥10): `pnpm install` → `pnpm dev` (HMR via `nodemon.json`), tests `pnpm test` / `pnpm test:e2e`, build `pnpm build`, preview `pnpm preview`.
