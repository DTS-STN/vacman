...
export const wfaStatusService: WFAStatusService = {
  ...
  async getById(id: string): Promise<Result<WFAStatus, AppError>> {
    ...
    try {
      const response = await apiFetch(`/wfa-statuses/${id}`, context);
      const data: WFAStatus = await response.json();
      return Ok(data);
    } catch (error) {
      if (error instanceof AppError && error.httpStatusCode === HttpStatusCodes.NOT_FOUND) {
        return Err(new AppError(`WFA statuses with ID '${id}' not found.`, ErrorCodes.NO_WFA_STATUS_FOUND));
      }
  ...
  async getByCode(code: string): Promise<Result<WFAStatus, AppError>> {
    ...
    try {
      ...
      if (error instanceof AppError && error.httpStatusCode === HttpStatusCodes.NOT_FOUND) {
        return Err(new AppError(`WFA statuses with CODE '${code}' not found.`, ErrorCodes.NO_WFA_STATUS_FOUND));
      }
      // Re-throw any other error
      throw error;
    }
