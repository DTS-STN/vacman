...
export async function action({ context, request }: Route.ActionArgs) {
  return withSpan('hiring-manager.requests.action', async (span) => {
    ...
    switch (action) {
      ...
      case 'copy': {
        ...
        const requestPayload: RequestUpdateModel = {
          ...
          languageRequirementIds: copiedRequestData.languageRequirements?.map((req) => ({ value: req.id })),
...
export async function loader({ context, request }: Route.LoaderArgs) {
  return withSpan('hiring-manager.requests.loader', async () => {
    const { session } = context.get(context.applicationContext);
    requireAuthentication(session, request);
      ...
      statusId:
        activeStatusIds.length > 0
          ? activeStatusIds.filter((id) =>
              REQUEST_STATUSES.filter((req) => req.category === REQUEST_CATEGORY.active).find(
