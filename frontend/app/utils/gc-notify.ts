...
export interface VerificationCodeRepository {
  ...
  sendVerificationCodeEmail(
    verificationCodeEmailRequestEntity: VerificationCodeEmailRequestEntity,
  ): Promise<VerificationCodeEmailResponseEntity>;
...
export class DefaultVerificationCodeRepository implements VerificationCodeRepository {
  ...
  async sendVerificationCodeEmail(
    ...
    const response = await this.httpClient.simpleFetch('http.client.interop-api.email-notifications.posts', url, {
      proxyUrl: this.serverConfig.HTTP_PROXY_URL,
      method: 'POST',
      headers: {
        'Authorization': `ApiKey-v1 ${this.serverConfig.GC_NOTIFY_API_KEY}`,
        'Content-Type': 'application/json',
        //'Ocp-Apim-Subscription-Key': this.serverConfig.INTEROP_API_SUBSCRIPTION_KEY,  // Uncomment if needed for subscription key
      },
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to 'POST' email notification. Response: ${response} `);
    }
  ...
  async checkHealth(): Promise<void> {
    ...
    const response = await this.httpClient.simpleFetch('http.client.interop-api.email-notifications.gets', url, {
      proxyUrl: this.serverConfig.HTTP_PROXY_URL,
      method: 'GET',
      headers: {
        'Authorization': `ApiKey-v1 ${this.serverConfig.GC_NOTIFY_API_KEY}`,
    ...
    if (!response.ok) {
      ...
      if (response.status === HttpStatusCodes.TOO_MANY_REQUESTS) {
        throw new AppError(
          'Failed to GET /notifications. Status: 429, Status Text: Too Many Requests',
          ErrorCodes.XAPI_TOO_MANY_REQUESTS,
        );
      }
...
export class MockVerificationCodeRepository implements VerificationCodeRepository {
  ...
  constructor() {
    this.log = LogFactory.getLogger('MockVerificationCodeRepository');
  }
  ...
  async sendVerificationCodeEmail(
    ...
    const verificationCodeEmailResponseEntity: VerificationCodeEmailResponseEntity = {
      ...
      uri: `https://api.example.com/v2/notifications/${id}`,
      template: {
        id: verificationCodeEmailRequestEntity.template_id,
        version: 1,
        uri: `https://api.example.com/v2/templates/${verificationCodeEmailRequestEntity.template_id}`,
      },
    };
